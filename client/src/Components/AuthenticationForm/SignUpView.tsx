...
const SignUpView = () => {
    ...
    const handleLogin = (e: any) => {
        e.preventDefault();
        setErrors({ ...errors, email: "Invalid email address", password: "Invalid password" });
    }

    return (
        <>
            <p className="flex flex-col text-center font-bold text-xl">Sign up<span className="text-sm font-medium text-gray-600">Please sign up to continue</span></p>
            ...
            <form className="flex flex-col gap-3 w-full">
                ...
                <Input error={errors.email} type="email" placeholder="Email address" text="Email address"/>

                <div className="flex gap-5">
                    <Input error="" type="text" placeholder="First Name" text="First Name"/>
                    <Input error="" type="text" placeholder="Last Name" text="Last Name"/>
                </div>

                <SelectBox items={country_list} title="Select Country" text="Country"/>

                {/* Todo: Implement password for this*/}
                <Input error={errors.password} type="text" placeholder="Password" text="Password"/>
                <div className="flex flex-col gap-3">
                    <GenericButton text="Sign up" className="shadow-sm h-[10%] bg-black! mt-3 font-semibold! text-white border-none text-[15px]!" onClick={handleLogin} />
            ...
            <p className="select-none font-[Outfit] text-[15px] text-gray-700">Already have an account? <span onClick={handleLoginRedirect} className="font-semibold cursor-pointer text-gray-900">Login</span></p>
        </>
    )
}
